...
namespace kx
{
    public class c : TcpClient
    {
        ...
        private static object[] NU = {
                                         ...
                                         , "", new DateTime(0),
                                         new Month(ni), new Date(ni), new DateTime(0), new KTimespan(nj), new Minute(ni)
                                         , new Second(ni), new TimeSpan(ni)
                                     };
        ...
        {
            for (int i = 0; i < NU.Length; i++) if (NU[i] != null && t == NU[i].GetType()) return NU[i];
            return null;
        }
        ...
        private static int[] nt = {0, 1, 0, 0, 1, 2, 4, 8, 4, 8, 1, 0, 8, 4, 4, 8, 8, 4, 4, 4}; // x.GetType().IsArray
        ...
        {
            if (!v6) throw new KException("Timespan not valid pre kdb+2.6");
        ...
        {
            if (!v6) throw new KException("Timestamp not valid pre kdb+2.6");
        ...
        private void read(byte[] b)
        {
            ...
            for (; i < n; i += j)
                if (0 == (j = s.Read(b, i, Math.Min(_maxBufferSize, n - i)))) throw new Exception("read");
        }
